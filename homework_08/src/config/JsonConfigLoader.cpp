...
void JsonConfigLoader::load() {
    std::ifstream file(filename);
        ...
        // ДЗ18: нові параметри секції simulation. Відсутні — беремо дефолти.
        if (jsonData.contains("simulation")) {
            const auto& sim = jsonData["simulation"];
            if (sim.contains("physicsTimeStep"))
                configBuilder.physicsTimeStep(sim["physicsTimeStep"].get<float>());
            if (sim.contains("timeScale"))
                configBuilder.timeScale(sim["timeScale"].get<float>());
        }
        file.close();
