...
std::unique_ptr<ITargetProvider> TargetProviderFactory::createTargetProvider(
    const Source sourceType, const std::string filename, float arrayTimeStep, float timeScale){
        if(sourceType == Source::JSON){
            return std::make_unique<ThreadSafeTargetProvider>(filename, arrayTimeStep, timeScale);
        }
        throw std::runtime_error("Unsupported file format: " + filename);
};
